...
class CommonAction extends Action{
	...
	public function getPos($colid=0,$key='',$date='',$search=''){
		$position = "<a href='".U('home/Index/index')."'>首页</a>";
		if($colid != 0){
			$where['colid'] = $colid;
	...
	public function getLinks($type){
		$obj = M('Links');
		$where['linktype'] = $type;
		$list = $obj->where($where)->order('ord asc')->select();
		return $list;
	}
	...
	public function getRandomList($num=10){
		...
		$count = $obj->count();
		if($count <= ($num*2)){
			$list = $obj->field('arcid,title')->order('rand()')->limit($num)->where($where)->select();
		}
		else{
			//使用此法可以在大数据量时，优化查询
			$rand = mt_rand(0,$count-$num-1);
			$list = $obj->field('arcid,title')->order('arcid desc')->limit($rand.','.$num)->where($where)->select();
		}
		foreach ($list as $k=>$v){
			$list[$k]['arcurl'] = U('Article/index',array('arcid'=>$v['arcid']));
		}
	...
	public function getTags(){
		...
		foreach ($list as $k=>$v){
			$tagname = urlencode($v['tagname']);//中文需要做编码处理
			$list[$k]['tagurl'] = U('Index/tag',array('key'=>$tagname));
		}
		return $list;
	}
	...
	public function getCFContent($model){
		...
		foreach ($list as $k=>$v){
			...
			$list[$k]['content'] = Input::nl2Br($v['content']);
		}
		return $list;
	}
	...
	public function saveCF($model,$prompt,$url){
		...
		//处理[code][/code]
		import("ORG.Util.Input");
		$content = htmlspecialchars(Input::getVar($_POST['content']));
		$content = preg_replace_callback("/\[code]((?:.|\n|\r)+?)\[\/code]/i",create_function('$matches','return "<pre>".$matches[1]."</pre>";'),$content);

		//输入过滤
		$writer = htmlspecialchars(Input::getVar($_POST['writer']));
		$email = htmlspecialchars(Input::getVar($_POST['email']));
		$obj->content =$content;
		$obj->writer = $writer;
		$obj->email = $email;
	...
	public function getColumnsName($colid){
		$col = M('Columns')->field('colname')->where(array('colid'=>$colid))->find();
		return $col['colname'];
	}
